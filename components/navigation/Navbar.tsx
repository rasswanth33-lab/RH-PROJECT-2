    ...
    >
      <div className="container-lab flex h-18 items-center justify-between py-4">
        <Link href="/" className="flex items-center gap-2.5" onClick={() => setOpen(false)}>
          <Image src="/brand/mark.png" alt="" width={40} height={40} className="rounded-sm" priority />
          ...
          >
            <nav className="container-lab flex flex-col gap-1 py-6">
              {links.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={cn("py-3 text-2xl font-medium tracking-tight", active ? "text-accent" : "text-ink")}
                  >
                    {link.label}
                  </Link>
                );
              ...
              <Link
                href="/contact"
                onClick={() => setOpen(false)}
                className="mt-4 inline-flex w-fit items-center gap-2 rounded-full bg-accent px-5 py-3 text-sm font-medium text-white"
              >
                Start a Project →
              </Link>
            </nav>
